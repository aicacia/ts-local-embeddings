...
function resolveBatchLimits(maxInputTokens: number): {
	...
	// Estimate how many worst-case-length documents fit in the token budget.
...
function estimateDocumentTokenLength(
	document: string,
	maxInputTokens: number,
): number {
	// Approximate tokenizer behavior to avoid a second tokenization pass per document.
...
export class LocalEmbeddings implements EmbeddingsInterface<number[]> {
	#tokenizer: Promise<TokenizerInstance>;
	#model: Promise<ModelInstance>;

	constructor(runtime: LocalEmbeddingsRuntime) {
		this.#tokenizer = Promise.resolve(runtime.tokenizer);
		this.#model = Promise.resolve(runtime.model);
	}
	...
	async embedDocuments(documents: string[]): Promise<number[][]> {
		...
		const flushBatch = async (): Promise<void> => {
			...
			const projectedDocumentsProcessed = documentsProcessed + currentBatchSize;
