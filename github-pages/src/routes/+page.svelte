...
<div class="mx-auto flex h-full min-h-0 max-w-5xl flex-1 flex-col px-4 py-8">
	...
	<p class="mt-1 text-slate-600">
		...
		{:else if indexing}
			Indexing {documents.length} documents in memory ({indexedDocumentCount}/{indexedDocumentTotal},
			{indexProgressPercent.toFixed(1)}%)
		{:else}
	...
	{#if modelLoading}
		<p
			class="mt-4 rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-sky-800"
		>
	...
	{:else if indexing}
		...
		>
			...
			<span class="mt-1 block text-sky-900">
				Indexed {indexedDocumentCount} of {indexedDocumentTotal} ({indexProgressPercent.toFixed(
					1,
				)}%)
			</span>
		</p>
	{:else if error}
		<p
			class="mt-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-red-700"
		>
			{error}
		</p>
	{/if}
	...
	{#if lastSearchDurationMs !== null}
		<p class="mt-6 text-sm text-slate-600">
			Results in {formatDuration(lastSearchDurationMs)}.
		</p>
	{/if}
	...
	<section class="mt-8 flex min-h-0 flex-1 flex-col">
		...
		>
			{#each filteredDocuments as item (String(item.document.metadata?.id ?? item.document.pageContent))}
				<li
					class="grid grid-cols-[auto_1fr] gap-2 rounded-lg border border-slate-200 bg-white p-3"
				>
					<strong class="text-slate-500"
						>#{String(item.document.metadata?.id ?? "-")}</strong
					>
					<div class="min-w-0">
